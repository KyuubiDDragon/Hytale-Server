    ...
    <div v-if="modUpdateStatus && modUpdateStatus.totalTracked > 0" class="card">
      <div class="card-header flex items-center justify-between">
        <h3 class="text-lg font-semibold text-white flex items-center gap-2">
          ...
          {{ t('dashboard.modUpdates.title') }}
        </h3>
        <div class="flex items-center gap-2">
          <button
            @click="checkModUpdates"
            :disabled="checkingModUpdates"
            class="px-3 py-1 text-sm bg-dark-100 text-gray-400 hover:text-white rounded transition-colors disabled:opacity-50"
          >
            <span v-if="checkingModUpdates">{{ t('dashboard.modUpdates.checking') }}</span>
            <span v-else>{{ t('dashboard.modUpdates.checkNow') }}</span>
          </button>
          <button
            @click="goToMods"
            class="px-3 py-1 text-sm bg-orange-500/20 text-orange-400 hover:bg-orange-500/30 rounded transition-colors"
          >
            {{ t('dashboard.modUpdates.viewAll') }}
          </button>
        </div>
      </div>
      <div class="card-body">
        <div class="flex items-center gap-4">
          <div
            ...
            :class="modUpdateStatus.updatesAvailable > 0 ? 'bg-orange-500/20' : 'bg-green-500/20'"
          >
            <svg
              v-if="modUpdateStatus.updatesAvailable > 0"
          ...
          <div>
            <p
              ...
              :class="modUpdateStatus.updatesAvailable > 0 ? 'text-orange-400' : 'text-green-400'"
            >
              <span v-if="modUpdateStatus.updatesAvailable > 0">
                {{ t('dashboard.modUpdates.updatesAvailable', { count: modUpdateStatus.updatesAvailable }) }}
              </span>
              <span v-else>{{ t('dashboard.modUpdates.allUpToDate') }}</span>
            </p>
            <p class="text-sm text-gray-400">
              {{ modUpdateStatus.totalTracked }} {{ t('dashboard.modUpdates.title').toLowerCase() }} tracked
            </p>
            <p v-if="modUpdateStatus.lastChecked" class="text-xs text-gray-500 mt-1">
              Last checked: {{ new Date(modUpdateStatus.lastChecked).toLocaleString() }}
            </p>
          </div>
        </div>

        <!-- List mods with updates -->
        <div v-if="modUpdateStatus.updatesAvailable > 0" class="mt-4 pt-4 border-t border-dark-50/30 space-y-2">
          <div
            v-for="mod in modUpdateStatus.mods.filter(m => m.hasUpdate)"
            :key="mod.filename"
          ...
          >
            <div class="flex items-center gap-3">
              <img
                v-if="mod.thumbnail"
                :src="mod.thumbnail"
                :alt="mod.projectTitle || mod.filename"
                class="w-10 h-10 rounded object-cover"
              />
              <div v-else class="w-10 h-10 rounded bg-dark-200 flex items-center justify-center">
